...
public class PC_AddieTest : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if ((groundedjump || walljumpright || walljumpleft) && Input.GetAxis ("Jump") > 0) 
		{
			newvelocity.y = jumpspeed;
			if (walljumpright) {
				newvelocity.x += wallpushspeed;
			}
			if (walljumpleft) {
				newvelocity.x += -wallpushspeed;
			}
	...
	void OnCollisionStay2D(Collision2D other) {
		...
		// if ( OnLeftOf (other.gameObject.GetComponent<Collider2D>() ) ){
		// 	walljumpleft = true;
		// }
		// if ( OnRightOf (other.gameObject.GetComponent<Collider2D>() ) ){
		// 	walljumpright = true;
		// }
	}
